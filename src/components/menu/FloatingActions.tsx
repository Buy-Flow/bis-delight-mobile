...
export function FloatingActions() {
  ...
  const bottomStyle = { bottom: "calc(env(safe-area-inset-bottom) + 6rem)" } as const;

  return (
    <>
      {/* WhatsApp — agora à esquerda */}
      <div className="fixed left-4 z-30 md:bottom-6" style={bottomStyle}>
        <a
          href={wa}
          target="_blank"
          rel="noreferrer"
          aria-label="Falar no WhatsApp"
          className="grid h-14 w-14 place-items-center rounded-full bg-[#25D366] text-white shadow-2xl touch-manipulation [-webkit-tap-highlight-color:transparent] transition-transform duration-100 ease-out active:scale-95 animate-pulse-glow"
        >
      ...
      <div
        className={`fixed right-4 z-30 md:bottom-6 transition-all duration-300 ${
      ...
      >
        <button
          ...
          className="flex items-center gap-2 rounded-full bg-neon-pink px-4 h-14 text-white font-bold shadow-2xl glow-pink touch-manipulation [-webkit-tap-highlight-color:transparent] transition-transform duration-100 ease-out active:scale-95"
        >
          <UtensilsCrossed className="h-5 w-5" />
          <span className="text-sm uppercase tracking-wider">Ver cardápio</span>
        </button>
      </div>
    </>
  );
