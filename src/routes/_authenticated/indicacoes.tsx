...
function IndicacoesAdmin() {
  ...
  const load = async () => {
    const [cfg, list] = await Promise.all([
      ...
      // @ts-expect-error rpc
      supabase.rpc("admin_list_referrals", { _limit: 200 }),
    ]);
            ...
            <Card className="bg-white/5 border-white/10 p-5 space-y-5">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-3 rounded-lg border border-white/10 p-4">
                  <div className="text-sm font-semibold text-fuchsia-300">🎁 Cupom do indicado (novo cliente)</div>
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(v: "fixed" | "percent") =>
                          setS({ ...s, referee_discount_type: v })
                        }
                      >
                ...
                {/* REFERRER */}
                <div className="space-y-3 rounded-lg border border-white/10 p-4">
                  <div className="text-sm font-semibold text-emerald-300">🏆 Cupom de quem indicou</div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <Label className="text-white/70 text-xs">Tipo</Label>
                      <Select
                        ...
                        onValueChange={(v: "fixed" | "percent") =>
                          setS({ ...s, referrer_discount_type: v })
                        }
                      >
                        <SelectTrigger className="bg-white/5 border-white/15 text-white">
                          <SelectValue />
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setS({ ...s, referrer_min_order: Number(e.target.value) })}
                      className="bg-white/5 border-white/15 text-white"
                    />
                  </div>
                </div>
                ...
                <div className="flex items-end">
                  <label className="flex items-center gap-2 text-white/80 text-sm">
                    <Switch
                      ...
                      onCheckedChange={(v) => setS({ ...s, require_first_order: v })}
                    />
              ...
              <div className="flex justify-end">
                <Button onClick={save} disabled={saving} className="bg-fuchsia-500 hover:bg-fuchsia-600 gap-2">
                    ...
                    <tbody className="divide-y divide-white/10">
                      {rows.map((r) => (
                        <tr key={r.id} className="text-white/80">
                          ...
                          <td className="py-2">
                            <Badge className={`${statusTone[r.status] ?? statusTone.signed_up} border-0`}>
                              {r.status}
                            </Badge>
                          </td>
                        </tr>
